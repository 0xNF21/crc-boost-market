...
function normalizeImageUrl(raw: unknown) {
  ...
  if (raw.startsWith("ipfs://")) return `${IPFS_GATEWAYS[0]}${raw.replace("ipfs://", "")}`;
  return raw;
}
...
async function fetchProfilesFromService(path: string, params: Record<string, string>) {
  try {
...
async function getProfileFromServiceByCid(cid: string): Promise<CirclesProfile | null> {
...
async function getProfileFromServiceByAddress(address: string): Promise<CirclesProfile | null> {
...
async function fetchProfile(address: string): Promise<CirclesProfile> {
  ...
  const result = {
    name: serviceProfile?.name || directProfile?.name || serviceCidProfile?.name || ipfsProfile?.name || "",
    imageUrl:
...
export async function POST(req: Request) {
  try {
    ...
    const limited = addresses.slice(0, 50).filter((address): address is string => typeof address === "string");
