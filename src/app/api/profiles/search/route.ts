...
async function searchProfilesFromService(query: string) {
  try {
    ...
    return results.filter((result) => result !== null).slice(0, 8);
  } catch {
    return [];
  }
}
...
async function searchProfiles(query: string) {
  ...
  try {
    ...
    const data = await res.json();
    const profiles: LegacySearchProfile[] = Array.isArray(data?.result) ? data.result : [];
    const filtered = profiles.filter((profile) => profile?.avatarType === "CrcV2_RegisterHuman").slice(0, 8);
