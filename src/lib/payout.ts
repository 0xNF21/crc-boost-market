...
import { botState, payouts } from "@/lib/db/schema";
...
export function getPayoutConfig(): PayoutConfig {
  ...
  if (missing.length > 0) return { configured: false, missingVars: missing };
...
async function reserveNonce(): Promise<number> {
  ...
  if (!row || typeof row.last_nonce !== "number") throw new Error("bot_state row missing");
...
async function execViaRolesMod(
  ...
  try {
    const tx = await rolesMod.execTransactionWithRole(targetAddress, value, calldata, 0, getRoleKey(), true, { nonce });
    return { hash: tx.hash };
  } catch (err: any) {
...
export async function executePayout(request: PayoutRequest): Promise<PayoutResult> {
  ...
  if (existing.length > 0) {
    ...
    if (ex.status === "sending") {
      return { success: false, status: "already_sending", error: `Payout already broadcast for gameId ${request.gameId}`, payoutId: ex.id, transferTxHash: ex.transferTxHash || undefined };
    }
  ...
  let payoutRecord: typeof payouts.$inferSelect;
  if (existing.length > 0 && existing[0].status === "failed") {
    payoutRecord = existing[0];
    await db.update(payouts).set({ status: "pending", errorMessage: null, updatedAt: new Date() }).where(eq(payouts.id, payoutRecord.id));
  ...
  await db.update(payouts).set({ attempts: currentAttempt, updatedAt: new Date() }).where(eq(payouts.id, payoutRecord.id));
  ...
  try {
    ...
    await db.update(payouts).set({ status: "sending", updatedAt: new Date() }).where(eq(payouts.id, payoutRecord.id));
    ...
    return { success: true, payoutId: payoutRecord.id, status: "sending", transferTxHash };
  } catch (error: any) {
    await db.update(payouts).set({
      status: "failed",
    ...
    return { success: false, payoutId: payoutRecord.id, status: "failed", error: error.message };
  }
}
