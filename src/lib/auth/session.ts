...
const DEFAULT_AUTH_DOMAIN = "nf-society.vercel.app";
...
async function verifySignature(
  ...
  // Layer 4: Circles host "raw" signatures can be checked against the raw
  // message bytes hash. Keep this after the canonical EIP-191 paths.
  try {
    const provider = new ethers.JsonRpcProvider(GNOSIS_RPC);
    const contract = new ethers.Contract(address, ERC1271_ABI, provider);
    ...
    console.warn("[auth] manual raw ERC-1271 threw:", err);
  }
 ...
 *
 * In dev (non-HTTPS) we also use SameSite=Lax. Browsers reject
 * SameSite=None cookies unless Secure is present, so `None + !Secure` makes
 * the auth response look successful while the session cookie is silently
 * discarded on localhost.
 */
